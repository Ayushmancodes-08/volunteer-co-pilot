...
import { translateRequestSchema } from '../validators/index';
...
const MOCK_TRANSLATIONS: Record<string, Record<string, TranslationEntry>> = {
  spanish: {
 ...
 */
async function translateText(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    ...
    request.log.error(
      { msg: 'Translation GenAI failed, using fallback dictionary', lang: parsed.targetLanguage, error: errMsg }
    );

    const lang = parsed.targetLanguage.toLowerCase();
    const intent = parsed.intent || 'general_info';

...
import { AIBriefingData, VolunteerProfile } from '../types';

interface AIBriefingProps {
  briefing: AIBriefingData | null;
  loading: boolean;
  onFetch: (name: string, role: string, gate: string) => Promise<void>;
  volunteer: VolunteerProfile | null;
}

export default function AIBriefing({ briefing, loading, onFetch, volunteer }: AIBriefingProps) {
  const { t } = useI18n();

  useEffect(() => {
    if (volunteer && !briefing && !loading) {
      void onFetch(volunteer.name, volunteer.role, volunteer.gate);
    }
